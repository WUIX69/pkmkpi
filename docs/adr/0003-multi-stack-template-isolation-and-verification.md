...
3. `templates/renzo` had HTML files nested in `html/` while referencing styles in `css/`, causing 404 path breaks when served at root.
4. `templates/leonor-olivera` required npm packages that could pollute root pnpm workspace dependencies.
...
4. **Renzo Path Normalization**: Flatten `templates/renzo/html/*.html` to `templates/renzo/*.html`, provide `templates/renzo/index.html`, and supply the PKMKPI seal in `templates/renzo/images/`.
...
6. **Out-of-Process Server Orchestration**: Implement `scripts/templates.mjs` managing dedicated ports:
...
8. **Multi-Viewport Automated Audits**: Provide `templates/tests/audit_all_templates.py` asserting HTTP 200, zero console errors, and zero horizontal scrollbar overflow across 6 standard viewports.
